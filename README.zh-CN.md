...
> **Vitals 是 Minecraft 1.21.3 的纯客户端 Fabric Mod，为生物和玩家添加平滑、可配置的 RPG 风格头顶血条。** 只需在想看到 HUD 的客户端安装，服务器无需安装。

[English](README.md) · [在 Modrinth 下载 v1.0.2](https://modrinth.com/mod/zoyluo-vitals/versions) · [GitHub v1.0.1](https://github.com/zoyluoblue/Minecraft_Vitals/releases/tag/v1.0.1) · [反馈问题](https://github.com/zoyluoblue/Minecraft_Vitals/issues)

## 为什么选择 Vitals？

战斗信息应该一眼可读。Vitals 为附近生物和玩家提供清晰的 RPG 血条、精确生命数值、平滑过渡与延迟伤害拖尾；它不改变玩法、实体数据或服务器规则。

- **RPG 战斗可读性：** 世界空间血条、精确当前/最大生命、可选名称和黄色护甲进度条。
...
2. 从 [Modrinth 下载 Vitals v1.0.2](https://modrinth.com/mod/zoyluo-vitals/versions) 和兼容版本的 **Fabric API**。GitHub 暂时保留 [v1.0.1](https://github.com/zoyluoblue/Minecraft_Vitals/releases/tag/v1.0.1) 作为备用下载。
...
| 平台 | 快捷键 |
| --- | --- |
| Windows / Linux | **左 Alt + N** |
| macOS | **左 Option + N** |

快捷键使用方法：

1. 进入单人世界或多人服务器，并关闭已打开的菜单或聊天界面。
2. 按住**左侧** Alt/Option，再点按一次 **N**。右侧 Alt/Option 不会触发 Vitals。
3. 在“显示”分页中，通过“血条”开启或关闭全部头顶血条，通过“护甲条”显示或隐藏黄色护甲条；在“实体”分页中选择需要追踪的生物分类。
4. 点击“应用”可保存但不关闭面板，点击“完成”可保存并关闭；“取消”会放弃草稿，存在未应用修改时按 Escape 会先询问是否放弃。

如果 macOS 或其他应用把左 Option + N 注册成了全局快捷键，请先在对应系统设置或应用中关闭或改绑冲突快捷键，让 Minecraft 能收到这个组合键。
...
- 精确的当前/最大生命，可自定义小数精度。
- 可选的实体名称，显示在血条上方。
- 可选的**黄色护甲进度条**，显示在血条上方并在条内保留精确护甲值；护甲为 `0` 时自动隐藏。

默认会始终显示符合条件的满血实体。为了避免遮挡视线，Vitals 会隐藏本地玩家、隐身实体、旁观者和死亡实体。

### 选择要追踪的实体

可分别启用或关闭：玩家、原版 Boss、已驯服生物、中立生物、敌对生物、友好生物和其他生物。盔甲架以及方块展示、物品展示等非活体展示实体始终不会显示血条。未知 Mod 添加、但继承 `LivingEntity` 的生物，会使用通用的“其他生物”分类。
...
- 继承 `LivingEntity` 的 Mod 生物可走通用兼容路径，盔甲架和非活体展示实体会被排除；包括暮色森林在内的专用兼容，在完成定向测试前不会宣称完全支持。
...
当前暂不支持。纯客户端无法可靠取得每个远端实体的这些数据，因此 Vitals 只显示可从常规客户端状态安全读取的生命数据。
